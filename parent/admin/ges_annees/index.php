...
<main class="content" role="main" tabindex="-1" aria-live="polite">
  <h2>Gestion des années d'activité</h2>
  <form method="post" action="process_annee.php" class="mb-4">
    <div class="input-group" style="max-width:400px;">
      <input type="text" name="annee" class="form-control" placeholder="Nouvelle année (ex: 2025-2026)" required>
      <button type="submit" name="nouvelle_annee" class="btn btn-primary">Créer et archiver l'année</button>
    </div>
  </form>
  <?php if (isset($_GET['success']) && $_GET['success'] == 'annee-creee'): ?>
    <div class="alert alert-success">Nouvelle année créée et données archivées avec succès !</div>
  <?php elseif (isset($_GET['error']) && $_GET['error'] == 'annee-vide'): ?>
    <div class="alert alert-danger">Veuillez saisir une année valide.</div>
  <?php endif; ?>
  <!-- contenu de la page côté droite -->
 

  <!-- Liste des années archivées -->
  <h3 class="mt-5">Années archivées</h3>
  <table class="table table-bordered table-striped mt-3">
    ...
    <tbody>
      <?php
      $archives = $pdo->query("SELECT a.id, a.annee, a.date_archive FROM archives a ORDER BY a.date_archive DESC")->fetchAll(PDO::FETCH_ASSOC);
      foreach ($archives as $arch) : ?>
        <tr>
          <td><?= htmlspecialchars($arch['annee']) ?></td>
          <td><?= htmlspecialchars($arch['date_archive']) ?></td>
          <td>
            <a href="restore_annee.php?id=<?= $arch['id'] ?>" class="btn btn-warning btn-sm" onclick="return confirm('Restaurer cette année ? Toutes les données actuelles seront remplacées !')">Restaurer</a>
          </td>
        </tr>
      <?php endforeach; ?>
    </tbody>
...
<script>
  ...
  // sweet alerte feedback suppression paiement
  <?php if (isset($_GET['success']) && $_GET['success'] == 'paiement-supprime'): ?>
  <script>
    Swal.fire({
      icon: 'success',
      title: 'Paiement supprimé',
      text: 'Le paiement a bien été supprimé.',
  ...
  <?php if (isset($_GET['error']) && $_GET['error'] == 'suppression-echouee'): ?>
  <script>
    Swal.fire({
      icon: 'error',
      title: 'Erreur',
      text: 'La suppression du paiement a échoué.',
