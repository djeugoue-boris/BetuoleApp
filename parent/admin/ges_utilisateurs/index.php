...
<div class="modal fade" id="editModal" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog">
    <form method="post" action="update_admin.php" class="modal-content">
      <div class="modal-header bg-primary text-white">
      ...
      <div class="modal-body">
        ...
        <div class="mb-2">
          <label>Matricule</label>
          <input type="text" class="form-control" name="matricule" id="edit-matricule" required>
        </div>
        <div class="mb-2">
          <label>Nom</label>
...
<script>
  ...
  <?php if (isset($_GET['success']) && $_GET['success'] === 'utilisateur-ajoute'): ?>
<script>
  Swal.fire({
    icon: 'success',
    title: 'Utilisateur ajouté',
    text: 'Le nouvel utilisateur a bien été enregistré.',
...
<script>
  Swal.fire({
    icon: 'error',
    title: 'Erreur',
    text: '<?= $_GET['error'] === 'email-existe' ? 'Cet email est déjà utilisé.' : 'Tous les champs sont obligatoires.' ?>',
...
<?php if (isset($_GET['success']) && $_GET['success'] === 'utilisateur-modifie'): ?>
<script>
  Swal.fire({
    icon: 'success',
    ...
    button.addEventListener('click', () => {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          window.location.href = 'delete_utilisateur.php?id=' + id;
        }
      });
    });
  });
